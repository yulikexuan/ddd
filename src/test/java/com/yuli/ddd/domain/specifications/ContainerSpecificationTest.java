...
public class ContainerSpecificationTest {
	...
	private void createSingleFeatureSpecifications() {
		this.explosiveSpecification = ContainerSpecification.create(
				ContainerFeature.ARMORED);
		this.flammableSpecification = ContainerSpecification.create(
				ContainerFeature.VENTILATED);
		this.toxicSpecification = ContainerSpecification.create(
				ContainerFeature.BIOLOGICAL);
	}
	...
	public void is_Always_Satisfied_By_All_Purpose_Containers() {
		...
		// Given
		ISpecification<Container> allFeaturesSpecification =
				this.explosiveSpecification
						.and(this.flammableSpecification)
						.and(this.toxicSpecification);

		// When
		boolean satisfied = allFeaturesSpecification.isSatisfiedBy(
				this.allPurposeContainer);
	...
	public void either_Specification() {
		...
		// Given
		ISpecification<Container> eitherSpecification =
				this.explosiveSpecification.or(this.flammableSpecification);

		// When
		boolean satisfied_1 = eitherSpecification.isSatisfiedBy(
				this.biologicalContainer);

		boolean satisfied_2 = eitherSpecification.isSatisfiedBy(
				this.freshAirContainer);

		boolean satisfied_3 = eitherSpecification.isSatisfiedBy(
	...
	public void not_Specification() {
		...
		// Given
		ISpecification<Container> notToxicSpecification =
				this.toxicSpecification.not()
						.and(this.flammableSpecification)
						.and(this.explosiveSpecification);
		...
		// When
		boolean satisfied_1 = notToxicSpecification.isSatisfiedBy(
				this.allPurposeContainer);
		boolean satisfied_2 = notToxicSpecification.isSatisfiedBy(
				this.freshAirContainer);
		boolean satisfied_3 = notToxicSpecification.isSatisfiedBy(
				this.stableContainer);
		boolean satisfied_0 = notToxicSpecification.isSatisfiedBy(
				notToxicContainer);
